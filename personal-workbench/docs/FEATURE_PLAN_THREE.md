...
> 日期：2026-07-16  
> 范围：实施计划 + 进度跟踪。与代码冲突时以代码和 `PROJECT_HANDBOOK.md` 为准。
...
| B. 任务筛选 / 搜索 / 归档 | 未开始 | 见下文 B 节 |
| C. 产物徽章 + 报告到达通知 | 未开始 | 见下文 C 节 |
...
- [ ] 可在历史列表看到至少 2 个已保存周次并切换，内容不串。
- [ ] 脏草稿切换周次仍会先保存（沿用现逻辑）。
- [ ] 默认姓名只影响**新建**周报，不覆盖已有周报 author。
- [ ] `npm run test:all` 中周报相关通过。
...
- [ ] 搜索「医学」只显示课程/学校含该字的任务。
- [ ] 状态 chip「已暂停」与统计卡 paused 数字语义一致（统计仍全局）。
- [ ] 归档后默认不可见；筛选「已归档」可见且可取消归档。
- [ ] 归档不清除产物路径；重新打开任务流程不受影响。
- [ ] `npm test` + 相关 E2E 通过。
...
- [ ] 有 `chatLogPath` 的任务显示对话徽章 ready。
- [ ] 任务夹内放入 `cards.md` 后，任务卡在文件夹变更或重新进入中心后出现卡片徽章。
- [ ] 报告下载完成：前台 toast；后台系统通知（人工在 Windows 验证）。
- [ ] 无活动任务时的普通下载不误发「评估报告」通知。
- [ ] 安全：`listTaskFiles` 仍受 temp/tasks 边界约束，不新增路径穿越面。
