...
| V3.4 | 平台填写助手：cards.md 解析、卡片舱逐字段一键复制、平台表单注入预研 | ✅ 已提交（含 3 个用户反馈修复），待回归验收 + 合入 master |
| V3.5（候选） | 平台全自动填卡（视 V3.4 勘测结论）、提示词自动归位、任务文档快速归档 | 💭 构想 |

> 当前阶段：V3.4 已开发完成并提交（最新提交 `3c95d55`，2026-06-12），处于「收尾 + 待回归验收 + 待合入 master」。优先项：清理 2 个 P1 缺陷（见回归清单附表 #1/#2）。
